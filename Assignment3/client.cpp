...
void send_tcp_packet(int sock, struct sockaddr_in *dest_addr, int seq, int ack, int syn, int ack_flag) {
     // Create a buffer for the packet (IP header + TCP header)
    ...
    ip->id = htons(54321);      // Identification field (random value)
...
int main() {
   // Create a raw socket for TCP protocol
    ...
    if (sock < 0) {
     ...
     // Set the IP_HDRINCL option to include the IP header with raw packets
    int one = 1;
 ...
 // Check if the packet is a SYN-ACK from the server (source port, SYN and ACK flags, and sequence number)
